...
func (p *CARProcessor) findProfileRecord(carReader *carv2.BlockReader, did string) (*ProfileRecord, error) {
    for {
        ...
        if s := getStringNode(n, "displayName"); s != "" { rec.DisplayName = &s }
        if s := getStringNode(n, "description"); s != "" { rec.Description = &s }
        if s := getStringNode(n, "avatar"); s != "" { rec.Avatar = &s }
        if s := getStringNode(n, "banner"); s != "" { rec.Banner = &s }
        return rec, nil
    }
...
func getStringNode(n ipld.Node, key string) string {
    ...
    if v.Kind() != ipld.Kind_String { return "" }
    s, err := v.AsString()
    if err != nil { return "" }
    return s
}
