...
impl FuzzyMatcher {
    ...
    pub fn fuzzy_match(&mut self, haystack: &str, needle: &str) -> Option<FuzzyMatch> {
        ...
        let match_type = Self::classify_match_type(
            &haystack_normalized,
            &needle_normalized,
            &positions,
        );
    ...
    fn classify_match_type(haystack: &str, needle: &str, positions: &[u32]) -> MatchType {
        ...
        if positions.len() > 0 {
            ...
            for word in &words {
                ...
                while word_start < haystack.len() && haystack.chars().nth(word_start).map_or(false, |c| c.is_whitespace()) {
                    word_start += 1;
                }
            }
        ...
        if first_pos == 0 || haystack.chars().nth(first_pos - 1).map_or(false, |c| !c.is_alphanumeric()) {
