...
fn read_header(reader: &mut SyncByteReader) -> Result<CarHeader, CarError> {
    ...
    let version = header_map.get(&serde_cbor::Value::Text("version".to_string()))
        .and_then(|v| match v {
    ...
    if version != 1 {
        return Err(CarError::InvalidHeader(format!("Unsupported version: {}", version)));
    }
    
    let roots_value = header_map.get(&serde_cbor::Value::Text("roots".to_string()))
