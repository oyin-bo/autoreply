...
mod tests {
    ...
    async fn test_submit_par_with_login_hint() {
        ...
        assert!(true, "OAuth manager can be created with optional handle support");
    }
    ...
    async fn test_submit_par_without_login_hint() {
        ...
        assert!(true, "OAuth manager accepts None handle for account selection");
    }

    #[test]
    fn test_pkce_generation() {
        // Verify PKCE generation produces valid values
        let (verifier, challenge) = crate::auth::oauth_atproto::AtProtoOAuthManager::generate_pkce();
        
        assert!(!verifier.is_empty(), "Code verifier should not be empty");
        assert!(!challenge.is_empty(), "Code challenge should not be empty");
        assert_ne!(verifier, challenge, "Verifier and challenge should be different");
        
        // Base64 URL-safe characters only
        assert!(verifier.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-'));
        assert!(challenge.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-'));
    }
    ...
    fn test_state_generation() {
        ...
        // Base64 URL-safe characters only
        assert!(state1.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-'));
    }
}
