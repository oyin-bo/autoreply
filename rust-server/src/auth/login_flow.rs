            ...
            let handle_str = match handle {
                ...
                _ => {
                    return Err(AppError::InvalidInput(
                        "Handle is required for app password authentication".to_string(),
                    ))
                }
            };
                ...
                if request.interactive {
                    ...
                    Ok(LoginOutcome {
                        ...
                        elicitation: Some(LoginElicitation {
                            field: "password".to_string(),
                            message: format!(
                                "OAuth failed. Enter app password for @{}",
                                handle_for_password
                            ),
                        }),
                    })
                    ...
                    match oauth_manager.complete_flow(&code, &flow_state).await {
                        Ok(mut session) => {
                            ...
                            if let Err(e) = storage.store_credentials_with_fallback(
                                ...
                                warn!(
                                    "OAuth background task: Failed to store credentials: {}",
                                    e.message()
                                );
                                return;
                            }
                            ...
                            {
                                warn!(
                                    "OAuth background task: Failed to store session: {}",
                                    e.message()
                                );
                                return;
                            }
                            ...
                            if let Err(e) = ensure_default(&storage, &session.handle) {
                                warn!(
                                    "OAuth background task: Failed to set default: {}",
                                    e.message()
                                );
                            }

                            debug!(
                                "OAuth background task: Successfully authenticated as @{}",
                                session.handle
                            );
                        }
                        Err(e) => {
                            warn!(
                                "OAuth background task: Failed to complete flow: {}",
                                e.message()
                            );
                        }
                    }
